...
import { DetailFields, DetailLink, TechnicalDetails } from "./DetailPrimitives";
...
export function ConflictCard({ detail }: { detail: LedgerConflictDetail }) {
  ...
  return <article className="detail-record" aria-label="账本冲突证据">
    ...
    {conflict.reminder_ignored && conflict.status === "OPEN" && <p className="detail-caution">提醒已忽略，冲突及其约束仍然有效。</p>}
    ...
    <div className="detail-record-footer"><div className="detail-actions">{conflict.status === "OPEN" && conflict.conflict_type !== "RAW_PAGE_VARIANT" && <Button variant="danger" disabled={completed} onClick={() => setConfirm(true)}>{label(action)}</Button>}{completed && <span role="status">冲突处理已记录。</span>}</div>{existing && <DetailLink to={"/reconciliation/ledger/" + existing.ledger_entry_id} label="打开流水详情" />}</div>
    <TechnicalDetails data={detail} identifiers={[["冲突编号", conflict.conflict_id]]} label="冲突技术详情" />
    {confirm && <ReasonDialog title={label(action)} description={action === "KEEP_EXISTING" ? "保留已经入账的流水，拒绝本次冲突记录。不会以新记录覆盖历史账本。" : "确认该无效流水保持隔离，不将它计入可分配的收款。"} action="确认处理冲突" onClose={() => setConfirm(false)}
