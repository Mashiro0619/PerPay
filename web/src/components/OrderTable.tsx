...
export function OrderTable({ orders, compact = false }: { orders: AdminOrderSummary[]; compact?: boolean }) {
  ...
  return <div className="table-scroll order-table-scroll" role="region" aria-label="订单列表" tabIndex={0}><table className={`data-table order-table ${compact ? "order-table--compact" : ""}`} role="table"><thead role="rowgroup"><tr role="row">
    ...
    <td className="order-identity" role="cell"><Link className="table-primary" data-row-link title={order.product_name} to={`/orders/${order.order_id}`}>{order.product_name}</Link><span className="table-secondary mono" title={order.merchant_order_no}>{order.merchant_order_no}</span>{order.refund_mark.marked && <span className="table-secondary"><Badge value="ADMIN_REFUND_MARK" label="已退款（管理员标记）" /></span>}</td>
