...
import { api, result, type RuntimeSettings } from "../api/client";
...
export function SettingsEditor({ section, settings, onSaved, guided = false, submitLabel = "保存配置" }: {
  ...
  return <>
    ...
    <Panel title={sections.find(([value]) => value === section)?.[1] ?? "设置"} className="content-panel">
      <form ref={draft.form} className="form-stack" onSubmit={submit} onChange={() => { draft.onChange(); setSavedMessage(null); }} autoComplete="off"><fieldset disabled={save.isPending}>
        {section === "provider" && <>
          ...
          {!guided && <details className="form-disclosure"><summary>导入已有应用私钥（可选）</summary><Field label="应用私钥" hint="留空使用已生成的私钥。替换密钥前，请确认支付宝平台中的应用公钥已同步。"><textarea name="private_key" rows={4} maxLength={16384} autoComplete="off" spellCheck={false} /></Field></details>}
        ...
        {section === "collection" && <>
          <CollectionCodeField defaultValue={settings.collection?.code_payload ?? ""} onDecoded={draft.onChange} onPendingChange={setDecoding} disabled={save.isPending} />
