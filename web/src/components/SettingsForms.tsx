...
export function SettingsEditor({ section, settings, onSaved, guided = false, submitLabel = "保存", secondaryAction }: {
  ...
  const canSave = guided || draft.dirty || (section === "provider" && !settings.completion.provider) || (section === "collection" && !settings.completion.collection);
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    submitting.current = true; setFieldErrors({}); setSavedMessage("");
  ...
  const editor = <form ref={draft.form} className="form-stack" onSubmit={submit} onInvalidCapture={(event) => { if (event.target instanceof HTMLElement) revealField(event.target); }} onChange={(event) => {
    draft.onChange(); setSavedMessage(""); save.reset();
    ...
    {section === "provider" && <>
      {!guided && <Notice>应用公钥上传至支付宝；支付宝公钥填在下方，两者不能混用。</Notice>}
      <div className="form-stack"><Field label="应用 ID（App ID）" error={fieldErrors.app_id}><input name="app_id" required maxLength={64} pattern="[A-Za-z0-9._-]+" defaultValue={settings.provider?.app_id ?? ""} autoComplete="off" placeholder="支付宝开放平台的应用 ID" /></Field></div>
      ...
      <AdvancedFields collapsed label={settings.provider?.environment === "SANDBOX" ? "高级设置 · 沙箱环境" : "高级设置"}><div className="form-grid">{environment}<NumberField name="timeout_milliseconds" error={fieldErrors.timeout_milliseconds} label="请求超时（毫秒）" value={settings.provider?.timeout_milliseconds ?? 8000} min={1000} max={120000} /><NumberField name="scan_interval_seconds" error={fieldErrors.scan_interval_seconds} label="常规采集间隔（秒）" value={settings.provider?.scan_interval_seconds ?? 30} min={5} max={3600} hint="空闲时使用；有效时限至少为此间隔的两倍。" />
        <NumberField name="active_scan_interval_seconds" error={fieldErrors.active_scan_interval_seconds} label="活跃采集间隔（秒）" value={settings.provider?.active_scan_interval_seconds ?? settings.provider?.scan_interval_seconds ?? 5} min={5} max={3600} hint="待支付及收尾时使用，不得大于常规间隔。" />
    ...
    {section === "collection" && <>
      <CollectionCodeField defaultValue={settings.collection?.code_payload ?? ""} onDecoded={() => { draft.onChange(); setFieldErrors({}); setSavedMessage(""); }} error={fieldErrors.code_payload} onPendingChange={setDecoding} disabled={save.isPending} />
      <NumberField name="order_ttl_seconds" error={fieldErrors.order_ttl_seconds} label="收银台有效期（秒）" value={settings.collection?.order_ttl_seconds ?? 300} min={60} max={1800} />
      <AdvancedFields collapsed><div className="form-grid"><NumberField name="amount_offset_maximum_cents" error={fieldErrors.amount_offset_maximum_cents} label="最大金额尾差（分）" value={settings.collection?.amount_offset_maximum_cents ?? 99} min={1} max={99} hint="用于区分同额订单，付款人需支付准确金额。" /><NumberField name="amount_reuse_cooldown_seconds" error={fieldErrors.amount_reuse_cooldown_seconds} label="金额复用冷却（秒）" value={settings.collection?.amount_reuse_cooldown_seconds ?? 600} min={60} max={3600} hint="结束后暂不复用应付金额，不延长订单有效期。" /></div></AdvancedFields>
      {!guided && <p className="field-hint">经营码变更只影响新订单。</p>}
    </>}
    {section === "notifications" && <>
      <p className="field-hint">付款后通知网站；关闭时需由网站主动查单。</p>
      ...
      <fieldset className="notification-fields" hidden={!notificationEnabled} disabled={!notificationEnabled}>
        <Field label="通知网站（HTTPS 域名）" error={fieldErrors.allowed_origin} hint="不含端口或路径，具体通知地址由订单指定。"><input name="allowed_origin" type="url" required={notificationEnabled} defaultValue={settings.notifications.allowed_origin ?? ""} placeholder="https://shop.example.com" /></Field>
      ...
      {!guided && <p className="field-hint">签名密钥在<Link to="/settings/security">密钥与安全</Link>中查看。</p>}
    </>}
    {section === "backup" && <>
      {!guided && <Notice>恢复需要数据库备份和主密钥，请一起保管。</Notice>}
    ...
    {section === "advanced" && <>
      <p className="field-hint">仅影响新订单，已有订单期限不变。</p>
    ...
    <div className={`form-actions settings-save${draft.dirty ? " is-dirty" : ""}`}><Button type="submit" variant="primary" pending={save.isPending} disabled={decoding || !canSave}>{!guided && <Save size={16} />}{submitLabel}</Button>{secondaryAction}
      {draft.dirty && <span className="field-hint" role="status">未保存</span>}<SuccessMessage message={savedMessage} />
    </div>
  </fieldset></form>;
...
export function ApplicationKey({ settings, onSaved, guided = false }: { settings: RuntimeSettings; onSaved: (settings: RuntimeSettings, message?: string) => void; guided?: boolean }) {
  ...
  return <Panel {...(guided || settings.application_public_key ? {} : { title: "应用公钥" })} className="content-panel application-key-panel">
    {settings.application_public_key ? guided ? <><p className="field-hint">应用公钥</p><CopyValue value={settings.application_public_key} label="复制应用公钥" /></> : <details className="form-disclosure"><summary>应用公钥<span className="field-hint">上传至支付宝开放平台</span></summary><CopyValue value={settings.application_public_key} label="复制应用公钥" /></details> : <Button variant="primary" pending={generate.isPending} onClick={() => requestDiscard(() => generate.mutate())}>{!guided && <KeyRound size={16} />}生成应用密钥</Button>}
