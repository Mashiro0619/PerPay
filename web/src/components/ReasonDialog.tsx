...
export function ReasonDialog({ title, description, action, children, execute, onClose, onSuccess }: {
  ...
  return <Dialog title={title} description={description} onClose={onClose} busy={mutation.isPending}>
    {children}
    <form className="form-stack" onSubmit={(event) => { event.preventDefault(); if (reason.trim() && !mutation.isPending) mutation.mutate(); }}>
      ...
      <div className="form-actions"><Button onClick={onClose} disabled={mutation.isPending}>取消</Button><Button type="submit" variant="danger" pending={mutation.isPending} disabled={!reason.trim()}>{action}</Button></div>
    </form>
  </Dialog>;
}
