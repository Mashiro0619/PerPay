...
function ReconciliationList({ section, status }: { section: Section; status: string }) {
  ...
  return <><div className="list-subtoolbar">{section === "exceptions" && <span>待处理异常</span>}<Button variant="quiet" pending={query.isFetching} onClick={() => { void query.refetch(); }}><RefreshCw size={14} />刷新</Button></div>
    ...
    </LinkedTableRow>)}</tbody></table></div> : <EmptyState title="暂无符合条件的记录" description="账本采集与自动对账运行后，相关记录会出现在这里。" />}<Pagination page={pagination.page} count={page.items.length} hasNext={!!page.page.next_cursor} pending={query.isFetching} onPrevious={pagination.previous} onNext={() => pagination.next(page.page.next_cursor)} /></>}</QueryView>
  </>;
}
