...
export default function Reconciliation() {
  ...
  return <><PageHeading title="账本与对账" actions={<Button variant="primary" onClick={() => setOperation(true)}>人工关联收款</Button>} />
    ...
    <Panel><div className="tabs" role="group" aria-label="对账记录类型"><SelectionIndicator active={section} underline />{([["matches", "支付关联"], ["exceptions", "账务异常"], ["conflicts", "账本冲突"]] as const).map(([value, text]) => <button type="button" key={value} aria-pressed={section === value} onClick={() => setSearch({ tab: value }, { replace: true })}>{text}</button>)}</div>
      ...
      </form>{section !== "exceptions" && <label><span className="sr-only">对账状态筛选</span><select value={status} onChange={(event) => setSearch({ tab: section, status: event.target.value }, { replace: true })}>{allowed.map((value) => <option key={value} value={value}>{({ SETTLED: "已关联", REVERSED: "已撤销", OPEN: "待处理（未忽略）", RESOLVED: "已处理", IGNORED: "已隔离", ALL: "全部记录（含已忽略）" } as Record<string, string>)[value]}</option>)}</select></label>}</div>
...
function ReconciliationList({ section, status }: { section: Section; status: string }) {
  ...
  return <><div className="list-subtoolbar"><span>{section === "exceptions" ? "待处理异常（未忽略）" : section === "conflicts" && status === "OPEN" ? "待处理冲突（未忽略）" : ""}</span>
    <div className="reminder-toolbar-actions">{section !== "matches" && <Link className="button button--quiet" to={"/work-items?type=" + (section === "exceptions" ? "FINANCIAL_EXCEPTION" : "LEDGER_CONFLICT") + "&visibility=IGNORED"}>查看已忽略</Link>}
