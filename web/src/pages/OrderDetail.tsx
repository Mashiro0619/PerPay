...
import { DetailFields, useDetailFetching, LazyDetails, TechnicalDetails } from "../components/detail/DetailPrimitives";
...
export function OrderDetail() {
  ...
  return <div className="detail-page order-detail-page"><p className="detail-eyebrow">订单详情</p>
...
function OrderDesk({ order }: { order: AdminOrderDetail }) {
  ...
  return <>
    <Panel className="detail-overview">
      <div className="detail-overview-top"><div className="detail-statuses"><Badge value={order.payment.status} /><Badge value={order.payment.basis} label={order.payment.basis === "NONE" ? "尚无付款依据" : undefined} /><Badge value={order.checkout.status} label={"收银台" + (order.checkout.status === "OPEN" ? "开放中" : order.checkout.status === "EXPIRED" ? "已过期" : "已关闭")} /></div></div>
      <div className="detail-amounts"><div className="detail-amount-primary"><span>实收金额</span><strong>{money(order.received_amount_cents)}</strong></div><div><span>应付金额（含尾差）</span><strong>{money(order.payable_amount_cents)}</strong></div><div><span>原始金额</span><strong>{money(order.requested_amount_cents)}</strong></div></div>
      <DetailFields className="detail-overview-facts" items={[["商户订单号", <CopyValue value={order.merchant_order_no} label="复制商户订单号" />], ["创建时间", dateTime(order.created_at)], ...(confirmed ? [["付款确认时间", dateTime(confirmed.occurred_at)] as const] : []), ["付款窗口截止", dateTime(order.checkout.expires_at)]]} />
    </Panel>
    <div className="order-detail-grid"><div className="order-detail-main">
      ...
      <Panel title="收款与依据" className="detail-panel">
        {active.map(match => <MatchCard key={match.payment_match_id} match={match} embedded />)}
        {!active.length && <div className="detail-empty-row"><p className="detail-empty">{reversed.length ? "原收款关联已撤销，当前没有有效关联。" : "尚无收款关联，订单付款状态以实际确认结果为准。"}</p>{order.payment.status === "UNPAID" && <AssociateIncomeAction orderId={order.order_id} orderLabel={order.product_name} />}</div>}
      ...
      <RefundMarkPanel key={order.order_id} order={order} />
      <TechnicalDetails data={order} identifiers={[["内部订单编号", order.order_id]]} label="订单技术详情" />
    </aside></div>
  </>;
}
