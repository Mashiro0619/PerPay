...
export function FinancialDialog({ initialOrderId = "", initialLedgerId = "", onClose, onSuccess }: {
  initialOrderId?: string; initialLedgerId?: string; onClose: () => void; onSuccess: () => void;
}) {
  ...
  return <Dialog title="人工关联收款" description="将已采集的一笔收入明确关联到订单。必须先核对订单及流水，不会自动推断你的决定。" onClose={onClose} busy={busy}>
    <form className="form-stack" onSubmit={submit}>
      ...
      <Field label="收入流水编号"><input name="ledger-entry-id" required pattern={resourceIdPattern.source} value={ledgerId} disabled={busy} onChange={(event) => { setLedgerId(event.target.value); resetPreview(); }} placeholder="UUID 格式的账本流水编号" /></Field>
      ...
      <div className="form-actions"><Button onClick={onClose} disabled={busy}>取消</Button><Button type="submit" variant={preview.data ? "danger" : "primary"} pending={busy} disabled={!!preview.data && (!directionValid || !accepted || !reason.trim())}>{!preview.data ? "读取并核对证据" : "确认关联收款"}</Button></div>
    </form>
  </Dialog>;
}
