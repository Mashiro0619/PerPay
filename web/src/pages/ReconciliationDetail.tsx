...
function ExceptionDetail({ resourceId }: { resourceId: string }) {
  ...
  return <><EvidenceHeading title="账务异常" section="exceptions" />{completed && <Notice tone="success">账务决定已保存。异常是否解决以重新读取的状态为准。</Notice>}<QueryView query={query}>{({ data }) => <>
    <Panel title={label(data.exception_type)} action={<Badge value={data.status} label={["UNMATCHED_DEBIT", "UNLINKED_REFUND"].includes(data.exception_type) ? "历史记录" : data.status === "OPEN" ? "待处理" : "已处理"} />} className="content-panel"><Details items={[["异常编号", <CopyValue value={data.exception_id} />], ["发现时间", dateTime(data.created_at)], ["关联订单", data.order_id ? <Link to={`/orders/${data.order_id}`}>{shortId(data.order_id)}</Link> : "尚未关联"], ["关联流水", data.ledger_entry_id ? <Link to={`/reconciliation/ledger/${data.ledger_entry_id}`}>{shortId(data.ledger_entry_id)}</Link> : "无"], ["匹配候选", data.candidate_id ? <Link to={`/reconciliation/candidates/${data.candidate_id}`}>{shortId(data.candidate_id)}</Link> : "无"], ["解决时间", dateTime(data.resolved_at)]]} />
      <JsonDetails data={data.details} label="查看异常证据" /><JsonDetails data={data.resolution} label="查看处理结果" />
      {["UNMATCHED_DEBIT", "UNLINKED_REFUND"].includes(data.exception_type) ? <Notice>这是历史支出提示，已不再作为待处理提醒。原始证据和状态保留，不表示问题已解决。</Notice> : data.status === "OPEN" && <div className="form-actions"><Button onClick={() => setOperation(true)}>人工关联收款</Button></div>}
    </Panel>{operation && <FinancialDialog initialOrderId={data.order_id ?? ""} initialLedgerId={data.ledger_entry_id ?? ""} onClose={() => setOperation(false)} onSuccess={() => { setOperation(false); setCompleted(true); }} />}
  </>}</QueryView></>;
}
