...
import { Badge, Button, Details, ErrorNotice, Field, Notice, PageHeading, Panel, QueryView } from "../components/ui";
...
function TestPaymentForm({ onNew }: { onNew: () => void }) {
  ...
  if (create.data) {
    ...
    return <Panel title="测试订单已创建" className="content-panel">
      ...
      {!checkoutUrl && <Notice tone="warning">收银台地址不可用，请打开订单查看。</Notice>}
  ...
  return <Panel className="content-panel">
    <QueryView query={status}>{({ data }) => <>
      ...
      <form onSubmit={submit} className="form-stack test-payment-form">
        <Field label="测试金额（元）" hint="真实付款，以收银台金额为准。" error={validation?.message}><input ref={amountField} name="amount" inputMode="decimal" autoComplete="off" required value={amount} disabled={create.isPending} onChange={(event) => { setAmount(event.target.value); setValidation(null); create.reset(); }} /></Field>
