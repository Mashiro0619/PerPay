...
export function AuthPage({ onLogin }: { onLogin: () => void }) {
  ...
  useEffect(() => { document.title = `${setup ? "初始化" : "登录"} · PerPay`; setError(null); setInvalidField(null); setPassword(""); setConfirmation(""); }, [setup]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (setup) {
        ...
        await result(api.loginAdministrator({ body: { password } }));
        setPassword("");
        onLogin();
