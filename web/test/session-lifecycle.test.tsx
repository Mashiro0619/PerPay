...
describe("administrator session lifecycle", () => {
  it.each(["network", "server"])(
    "retains unsaved settings through a temporary %s session failure",
    async (failure) => {
      ...
      vi.stubGlobal(
        ...
        vi.fn(async (request: Request) => {
          if (new URL(request.url).pathname.endsWith("/session")) {
            if (unavailable && failure === "network")
              throw new TypeError("temporary network outage");
            return unavailable
              ? apiError("temporary_unavailable", "temporary outage", 503)
              : json(session);
          }
  ...
  it.each([401, 403])(
    "still discards protected drafts for an explicit session rejection (%s)",
    async (status) => {
      vi.stubGlobal(
        "fetch",
        vi.fn(async (request: Request) =>
          new URL(request.url).pathname.endsWith("/session")
            ? apiError("session_invalid", "会话失效", status)
            : json({ data: settings }),
        ),
      );
      mount();
  ...
  it("never restores protected cache from a settings save completed after session expiry", async () => {
    ...
    const saved = {
      ...settings,
      revision: 4,
      collection: { ...settings.collection!, order_ttl_seconds: 450 },
    };
    await act(async () => {
      finish(json({ data: saved }));
    });
    expect(queryClient.getQueryData(["settings"])).toBeUndefined();
