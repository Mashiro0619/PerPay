          ...
          element={
            <table>
              <tbody>
                <LinkedTableRow>
                  <td>
                    <Link data-row-link to="/record">
                      记录标题
                    </Link>
                  </td>
                  <td>可选择的编号</td>
                  <td>
                    <Link to="/related">关联订单</Link>
...
describe("linked table row behavior", () => {
  it("opens the native primary link when clicking a non-link cell", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("cell", { name: "可选择的编号" }));
  ...
  it("does not override the associated order link", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("link", { name: "关联订单" }));
  ...
  it.each(["ctrlKey", "metaKey", "shiftKey", "altKey"] as const)(
    "forwards %s to the native link",
    (modifier) => {
      renderRow();
      const forwarded = vi.fn((event: Event) => event.preventDefault());
      screen
        .getByRole("link", { name: "记录标题" })
        .addEventListener("click", forwarded);
      fireEvent.click(screen.getByRole("cell", { name: "可选择的编号" }), {
        [modifier]: true,
      });
  ...
  it("forwards middle clicks but leaves right clicks alone", () => {
    ...
    screen
      .getByRole("link", { name: "记录标题" })
      .addEventListener("auxclick", forwarded);
    const cell = screen.getByRole("cell", { name: "可选择的编号" });
    fireEvent(
      cell,
...
const listCases = [
  ...
  },
  {
    ...
    element: <Reconciliation />,
    target: `/reconciliation/conflicts/${recordId}`,
    record: {
  ...
  },
  {
    ...
    element: <Reconciliation />,
    target: `/reconciliation/exceptions/${recordId}`,
    record: {
...
describe("detail navigation from actual list pages", () => {
  it.each(listCases)(
    "opens $name from the row without a separate action column",
    async ({ path, element, target, record, columns }) => {
      vi.stubGlobal(
        "fetch",
        vi.fn(async () =>
          json({ data: [record], page: { next_cursor: null } }),
        ),
      );
      ...
      expect(
        screen.queryByText("自动投递与人工重发均保留独立记录"),
      ...
      expect(
        within(table).queryByRole("link", { name: "详情" }),
      ).not.toBeInTheDocument();
      expect(
        within(table).queryByRole("link", { name: "查看证据" }),
      ).not.toBeInTheDocument();
