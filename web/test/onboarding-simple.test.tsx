...
describe("minimal onboarding", () => {
  ...
  it("does not mark a reverted notification toggle as dirty and preserves hidden input drafts", async () => {
    ...
    await user.type(screen.getByLabelText("通知网站（HTTPS 域名）"), "https://draft.example.com");
    await user.click(enabled); await user.click(enabled);
    expect(screen.getByLabelText("通知网站（HTTPS 域名）")).toHaveValue("https://draft.example.com");
  ...
  it("saves disabled notifications without submitting hidden invalid drafts or resetting retry defaults", async () => {
    ...
    const enabled = await screen.findByRole("checkbox", { name: "启用业务通知" });
    await user.click(enabled);
    await user.type(screen.getByLabelText("通知网站（HTTPS 域名）"), "invalid-origin");
    ...
    await user.click(enabled);
    expect(screen.getByLabelText("通知网站（HTTPS 域名）")).toHaveValue("invalid-origin");
  ...
  it("reads a secret once only after the explicit view action", async () => {
    ...
    expect(within(dialog).queryByRole("button", { name: "读取明文" })).not.toBeInTheDocument();
  ...
  it("keeps explicit confirmation when replacing a working API key", async () => {
    ...
    expect(screen.getByText(/旧密钥立即失效。轮换后需更新业务服务端/)).toBeVisible();
  ...
  it("does not automatically repeat first-time key generation after an uncertain response", async () => {
    ...
    expect(screen.getByText(/结果未确认，请关闭后刷新配置并查看当前密钥/)).toBeVisible();
