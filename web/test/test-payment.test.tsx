...
describe("minimal test payment page", () => {
  it("shows one amount field without redundant prompts and creates only on explicit submit", async () => {
    ...
    expect(
      screen.queryByText(/这不是模拟支付|随机尾差|我了解这会创建真实收款订单/),
    ).not.toBeInTheDocument();
    expect(
      view.container.querySelector(".test-payment-page .notice"),
    ).toBeNull();
    ...
    expect(await view.writes()[0]!.clone().json()).toEqual({
      amount_cents: 1,
      test_payment_id: expect.stringMatching(/^[0-9a-f-]{36}$/),
    });
    expect(screen.getByRole("link", { name: "打开收银台" })).toHaveAttribute(
      "href",
      checkoutUrl,
    );
    expect(screen.getByRole("link", { name: "打开收银台" })).toHaveAttribute(
    ...
    expect(
      view.container.querySelector(".test-payment-page .notice--success"),
    ).toBeNull();
  });

  it.each(["0", "-1", "100.01", "1.001", "invalid"])(
    "retains amount validation for %s without creating an order",
    async (value) => {
      const view = mount();
      const amount = await screen.findByLabelText("测试金额（元）");
      ...
      await userEvent
        .setup()
        .click(screen.getByRole("button", { name: "创建测试订单" }));
      expect(amount).toHaveFocus();
      expect(amount).toHaveAttribute("aria-invalid", "true");
      expect(
        screen.getByText(/金额须介于|请输入正确的人民币金额/),
      ).toBeVisible();
      expect(view.writes()).toHaveLength(0);
  ...
  it("keeps collection readiness blocking even without the confirmation checkbox", async () => {
    const view = mount((request) =>
      request.url.endsWith("/system/status")
        ? json({ data: { ...systemStatus(), status: "not_ready" } })
        : undefined,
    );
    const amount = await screen.findByLabelText("测试金额（元）");
    const submit = screen.getByRole("button", { name: "创建测试订单" });
    expect(submit).toBeDisabled();
    expect(screen.getByRole("link", { name: "完成配置" })).toHaveAttribute(
      "href",
      "/settings",
    );
  ...
  it("disables creation after a status refresh fails and preserves the entered amount", async () => {
    ...
    fail = true;
    await act(async () => {
      await queryClient.refetchQueries({ queryKey: ["status"], exact: true });
    });
  ...
  it("keeps the idempotency key after a network failure and resets it only for an explicit new order", async () => {
    ...
    fireEvent.change(screen.getByLabelText("测试金额（元）"), {
      target: { value: "1.23" },
    });
    await user.click(screen.getByRole("button", { name: "创建测试订单" }));
    await screen.findByRole("heading", { name: "测试订单已创建" });
    expect((await view.writes()[2]!.clone().json()).test_payment_id).not.toBe(
      first.test_payment_id,
    );
  });
  ...
  it("prevents duplicate submissions while the original request is pending", async () => {
    ...
    await userEvent
      .setup()
      .dblClick(screen.getByRole("button", { name: "创建测试订单" }));
  ...
  it("discards a late checkout result after the administrator session expires", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "测试订单已创建" }),
    ).not.toBeInTheDocument();
  });
});
