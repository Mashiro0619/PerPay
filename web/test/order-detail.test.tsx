...
import { queryClient, type AdminOrderDetail, type OrderWebhookDeliveryDetail } from "../src/api/client";
...
function Location() { return <output aria-label="当前页面">{useLocation().pathname}</output>; }
function mount(path = "/orders/" + orderId) {
  return render(<QueryClientProvider client={queryClient}><MemoryRouter initialEntries={[path]}><Routes>
    <Route path="/orders/:orderId" element={<OrderDetail />} /><Route path="/reconciliation/:kind/:resourceId" element={<EvidenceDetail />} />
  </Routes><Location /></MemoryRouter></QueryClientProvider>);
}
function mockData(initialOrder = paidOrder, initialDeliveries: OrderWebhookDeliveryDetail[] = [delivery], write?: (request: Request) => Promise<Response>) {
  const state = { order: initialOrder, deliveries: initialDeliveries };
  const requests: Request[] = [];
  vi.stubGlobal("fetch", vi.fn(async (request: Request) => {
    requests.push(request.clone());
    const url = new URL(request.url);
    if (request.method !== "GET") { if (write) return write(request); throw new Error("unexpected write"); }
    if (url.pathname === "/api/admin/v1/orders/" + orderId) return json({ data: state.order });
    if (url.pathname.endsWith("/notifications/deliveries")) return json({ data: state.deliveries, page: { next_cursor: null } });
    if (url.pathname.endsWith("/matches/" + matchId)) return json({ data: state.order.reconciliation.matches[0] ?? paymentMatch });
    if (url.pathname.endsWith("/candidates/" + candidateId)) return json({ data: candidate });
    if (url.pathname.endsWith("/ledger-entries/" + ledgerId)) return json({ data: detailLedger });
...
describe("order detail work surface", () => {
  it("shows payment evidence and notification outcomes in place without refetching embedded resources", async () => {
    const { requests } = mockData(); mount();
    expect(await screen.findByRole("heading", { name: order.product_name, level: 1 })).toBeVisible();
    expect(await screen.findByText(/HTTP 200 · 业务确认响应无效/)).toBeVisible();
  ...
  it("keeps ignored exceptions in expandable history and uses the embedded candidate and ledger", async () => {
    const { requests } = mockData({ ...paidOrder, reconciliation: { matches: [paymentMatch], exceptions: [{ ...financialException, reminder_ignored: true }] } }); mount();
    await screen.findByText(/HTTP 200/);
    expect(screen.queryByRole("heading", { name: "实收金额不符" })).not.toBeInTheDocument();
  ...
  it("shows manual actors and reversal reasons and never calls a settled timestamp a reversal", async () => {
    const creation = { ...operation, operation_type: "MANUAL_SETTLEMENT" as const, actor_type: "ADMIN" as const, actor_id: "管理员甲", reason: "对照交易号确认" };
    const reversal = { ...operation, operation_type: "REVERSE_SETTLEMENT" as const, actor_type: "ADMIN" as const, actor_id: "管理员乙", reason: "平台记录归属不符" };
    const reversed = { ...paymentMatch, evidence_type: "MANUAL" as const, status: "REVERSED" as const, candidate: null, creation_operation: creation, resolution_operation: reversal };
    mockData({ ...paidOrder, payment: { ...paidOrder.payment, status: "DISPUTED" }, reconciliation: { matches: [reversed], exceptions: [] } }); mount();
  ...
  it("reverses an association without navigation and refreshes the current order", async () => {
    const { state, requests } = mockData(paidOrder, [], async request => {
      expect(new URL(request.url).pathname).toBe("/api/admin/v1/reconciliation/matches/" + matchId + "/actions/reverse");
    ...
    expect(requests.filter(request => new URL(request.url).pathname === "/api/admin/v1/orders/" + orderId).length).toBeGreaterThan(1);
  });
  ...
  it("keeps order evidence usable when notification reads fail and refresh retries secondary data", async () => {
    let attempts = 0;
    vi.stubGlobal("fetch", vi.fn(async (request: Request) => {
      if (request.url.endsWith("/" + orderId)) return json({ data: paidOrder });
  ...
  it("opens a candidate in place with readable order and ledger facts and locks known association IDs", async () => {
    const openOrder: AdminOrderDetail = { ...order, reconciliation: { matches: [], exceptions: [] } };
    vi.stubGlobal("fetch", vi.fn(async (request: Request) => {
      const path = new URL(request.url).pathname;
      if (path.endsWith("/candidates/" + candidateId)) return json({ data: { ...candidate, status: "ELIGIBLE" } });
      if (path.endsWith("/orders/" + orderId)) return json({ data: openOrder });
      if (path.endsWith("/ledger-entries/" + ledgerId)) return json({ data: { ...detailLedger, state: "UNALLOCATED" } });
      throw new Error("unexpected request");
    })); mount("/reconciliation/candidates/" + candidateId); const user = userEvent.setup();
    await user.click(await screen.findByRole("button", { name: "人工关联收款" }));
    const dialog = screen.getByRole("dialog");
    expect(within(dialog).queryByLabelText("内部订单编号")).not.toBeInTheDocument();
  ...
  it("loads only the selected notification page and keeps historical redelivery disabled", async () => {
    const requests: Request[] = [];
    const old = { ...delivery, is_latest: false, delivery: { ...delivery.delivery, status: "ACKNOWLEDGED", delivery_id: "old-delivery" } };
    vi.stubGlobal("fetch", vi.fn(async (request: Request) => {
      requests.push(request.clone()); const url = new URL(request.url);
      if (url.pathname.endsWith("/" + orderId)) return json({ data: paidOrder });
      return json({ data: url.searchParams.has("cursor") ? [old] : [delivery], page: { next_cursor: url.searchParams.has("cursor") ? null : "next-delivery-page" } });
    })); mount(); const user = userEvent.setup();
    await screen.findByText(/HTTP 200/);
    expect(requests).toHaveLength(2);
  ...
  it("saves a refund declaration from the order without financial changes or navigation", async () => {
    const { state, requests } = mockData(paidOrder, [], async request => {
      ...
      expect(body).toEqual({ operation_id: expect.any(String), version: 0, marked: true, note: "外部退款已完成" });
      const mark = { marked: true, version: 1, note: body.note, updated_at: operation.created_at, updated_by: "admin" };
  ...
  it("rechecks known association facts and blocks confirmation if the order has since been paid", async () => {
    let orderReads = 0; const writes: Request[] = [];
    vi.stubGlobal("fetch", vi.fn(async (request: Request) => {
      if (request.method !== "GET") writes.push(request);
      if (request.url.endsWith("/candidates/" + candidateId)) return json({ data: { ...candidate, status: "ELIGIBLE" } });
      if (request.url.endsWith("/ledger-entries/" + ledgerId)) return json({ data: { ...detailLedger, state: "UNALLOCATED" } });
      orderReads += 1; return json({ data: orderReads === 1 ? order : paidOrder });
    })); mount("/reconciliation/candidates/" + candidateId);
    await userEvent.setup().click(await screen.findByRole("button", { name: "人工关联收款" }));
  ...
  it("refreshes both the order and notification result with the page refresh action", async () => {
    const { requests } = mockData(); mount(); const user = userEvent.setup();
    await screen.findByText(/HTTP 200/);
    await waitFor(() => expect(screen.getByRole("button", { name: "刷新" })).toBeEnabled());
    await user.click(screen.getByRole("button", { name: "刷新" }));
    await waitFor(() => expect(requests).toHaveLength(4));
    expect(requests.filter(request => new URL(request.url).pathname.endsWith("/" + orderId))).toHaveLength(2);
    expect(requests.filter(request => request.url.includes("/notifications/deliveries"))).toHaveLength(2);
  });
