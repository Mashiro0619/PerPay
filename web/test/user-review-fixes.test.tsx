...
describe("homepage collection health", () => {
  it.each(["ledger", "database", "reconciliation"] as const)(
    "shows a %s outage without sending a configured instance to onboarding",
    async (kind) => {
      const view = mountOnboarding({
        ...
        status: (settings) => {
          ...
          if (kind === "ledger") {
            ...
            status.ledger.last_error_code = "remote_authorization_failed";
          }
  ...
  it("keeps non-blocking warnings distinct from stopped payments and invalidates them on a failed read", async () => {
    ...
    mountOnboarding({
      ...
      handle: (request) =>
        fail && request.url.endsWith("/system/status")
  ...
  it("polls while visible, pauses while hidden, and reads afresh on return", async () => {
    ...
    const reads = () =>
      view.fetchMock.mock.calls.filter(([request]) =>
        request.url.endsWith("/system/status"),
      ).length;
    for (let attempt = 0; attempt < 10 && reads() === 0; attempt += 1)
      await act(async () => {
...
describe("guided configuration field errors", () => {
  it("does not submit arbitrary text as a collection code and focuses the retained input", async () => {
    const view = mountOnboarding({
      stage: 2,
      path: "/settings/onboarding/collection",
    });
  ...
  it("associates a server key error with the exact field without clearing the draft", async () => {
          ...
          ? json(
              {
                error: {
                  ...
                  message: "请改填支付宝公钥。",
                  request_id: "field-test",
                  fields: { platform_public_key: "请改填支付宝公钥。" },
                },
              },
              422,
            )
    ...
    fireEvent.change(await screen.findByLabelText("应用 ID（App ID）"), {
      target: { value: "app-id-test" },
    });
    const key = screen.getByLabelText("支付宝公钥");
    fireEvent.change(key, { target: { value: "invalid-public-key" } });
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "保存并继续" }));
    await waitFor(() => expect(key).toHaveFocus());
    expect(key).toHaveAttribute("aria-invalid", "true");
    expect(key).toHaveValue("invalid-public-key");
    expect(screen.getByLabelText("应用 ID（App ID）")).toHaveValue(
      "app-id-test",
    );
    expect(view.router.state.location.pathname).toBe(
      "/settings/onboarding/provider",
    );
    fireEvent.change(key, { target: { value: "corrected-draft" } });
  ...
  if (url.pathname.endsWith("/orders"))
    return json({
      data: [
        {
          ...order,
          product_name: url.searchParams.has("cursor")
            ? "第二页订单"
            : "第一页订单",
        },
      ],
...
describe("list navigation continuity", () => {
  it("clears both the filter and cursor when clearing filters on a later page", async () => {
    const view = mountOnboarding({
      stage: 4,
      path: "/orders?payment=UNPAID&cursor=opaque-cursor&page=2&source=overview",
      handle: orderRequests,
    });
    await screen.findByRole("link", { name: "第二页订单" });
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "清除筛选" }));
    expect(
      await screen.findByRole("link", { name: "第一页订单" }),
    ).toBeVisible();
    expect(view.router.state.location.search).toBe("?source=overview");
  });

  it.each(["page link", "browser back"])(
    "restores page two and filters via %s",
    async (returnMethod) => {
      const view = mountOnboarding({
        stage: 4,
        path: "/orders?payment=UNPAID&checkout=OPEN",
        handle: orderRequests,
      });
      const user = userEvent.setup();
      await user.click(await screen.findByRole("button", { name: "下一页" }));
      await user.click(await screen.findByRole("link", { name: "第二页订单" }));
      await screen.findByRole("heading", { name: order.product_name });
      if (returnMethod === "page link")
        await user.click(screen.getByRole("link", { name: "订单列表" }));
      else
        await act(async () => {
  ...
  it("restores a directly opened cursor and does not invent an unknown previous cursor", async () => {
    mountOnboarding({
      stage: 4,
      path: "/orders?payment=UNPAID&cursor=opaque-cursor&page=4",
      handle: orderRequests,
    });
    expect(
      await screen.findByRole("link", { name: "第二页订单" }),
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "返回首页" }));
    expect(
      await screen.findByRole("link", { name: "第一页订单" }),
    ).toBeVisible();
    expect(screen.getByLabelText("付款状态筛选")).toHaveTextContent("未付款");
