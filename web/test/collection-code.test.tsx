...
vi.mock("../src/qr-image", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../src/qr-image")>()),
  decodeQrImage: vi.fn(),
}));

const recognizedLink =
  "https://qr.alipay.com/fkx-upload-test?value=a%2Fb&amount=0.01";
const imageFile = () =>
  new File(["image"], "支付宝经营码.png", { type: "image/png" });
...
function mount(conflict = false) {
  const fetchMock = vi.fn(async (request: Request) => {
    if (request.method === "PUT") {
      if (conflict)
        return apiError("settings_revision_conflict", "配置版本冲突，请刷新");
      const { revision, ...collection } = await request.clone().json();
      return json({
        data: { ...settings, revision: revision + 1, collection },
      });
    }
...
describe("collection QR image upload", () => {
  it("rejects a decoded non-Alipay QR without replacing the current draft", async () => {
    vi.mocked(decodeQrImage).mockResolvedValue(
      "https://example.invalid/not-alipay",
    );
    const view = mount();
    const field = await screen.findByLabelText("支付宝经营码内容");
    fireEvent.change(field, {
      target: { value: "https://qr.alipay.com/retained-draft" },
    });
    await userEvent
      .setup()
      .upload(screen.getByLabelText("选择经营码二维码图片"), imageFile());
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "请使用支付宝经营码",
    );
    expect(field).toHaveValue("https://qr.alipay.com/retained-draft");
    expect(writes(view.fetchMock)).toHaveLength(0);
  });

  it("puts a clickable upload card before a separate compact editor", async () => {
    mount();
    const field = await screen.findByLabelText("支付宝经营码内容");
    const upload = screen.getByRole("button", { name: "上传二维码" });
    ...
    expect(
      upload.compareDocumentPosition(field) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).not.toBe(0);
    expect(field).toHaveAttribute("rows", "2");
    const choose = vi.spyOn(
      screen.getByLabelText("选择经营码二维码图片"),
      "click",
    );
  ...
  it.each(["picker", "drop"])(
    "fills the original link locally via %s and saves only after confirmation with the current revision",
    async (source) => {
      ...
      if (source === "picker")
        ...
        expect(dropFiles(uploadZone(), [imageFile()]).defaultPrevented).toBe(
          true,
        );
      ...
      expect(await writes(fetchMock)[0]!.json()).toEqual({
        revision: 3,
        code_payload: recognizedLink,
        order_ttl_seconds: 450,
        amount_offset_maximum_cents: 99,
        amount_reuse_cooldown_seconds: 1200,
      });
      expect(screen.getByLabelText("支付宝经营码内容")).toHaveValue(
        recognizedLink,
      );
  ...
  it("allows selecting the same image again and does not make an unchanged payload dirty", async () => {
    vi.mocked(decodeQrImage).mockResolvedValue(
      settings.collection!.code_payload,
    );
    mount();
  ...
  it("keeps existing content on failure and supports retrying the same file", async () => {
    vi.mocked(decodeQrImage)
      .mockRejectedValueOnce(
        new Error("未识别到二维码，请上传清晰、完整的二维码图片。"),
      )
      .mockResolvedValueOnce(recognizedLink);
    ...
    expect(
      screen.getByRole("button", { name: "上传二维码" }),
    ).toHaveAccessibleDescription(
      /未识别到二维码，请上传清晰、完整的二维码图片。/,
    );
  ...
  it("prevents saving the old payload while an image is being decoded", async () => {
    ...
    vi.mocked(decodeQrImage).mockReturnValue(pending.promise);
    const { fetchMock } = mount();
    const picker = await screen.findByLabelText("选择经营码二维码图片");
    await userEvent.setup().upload(picker, imageFile());
    expect(screen.getByRole("button", { name: "上传二维码" })).toHaveAttribute(
      "aria-busy",
      "true",
    );
    ...
    expect(writes(fetchMock)).toHaveLength(0);
    await act(async () => {
      pending.resolve(recognizedLink);
    });
  ...
  it.each([
    ...
    "never replaces a newer manual edit with a late %s decoder %s",
    async (source, outcome) => {
      ...
      await user.type(field, "https://qr.alipay.com/manual-edit");
      expect(signal.aborted).toBe(true);
      expect(screen.getByRole("button", { name: "保存" })).toBeEnabled();
      await act(async () => {
        if (outcome === "success") pending.resolve(recognizedLink);
  ...
  it.each(["picker", "drop"])(
    "ignores an older selection without ending a newer pending %s decode",
    async (source) => {
      ...
      if (source === "picker")
        ...
        dropFiles(screen.getByRole("button", { name: "上传二维码" }), [
          imageFile(),
        ]);
      expect(previousSignal.aborted).toBe(true);
      await act(async () => {
        first.resolve("https://qr.alipay.com/older-image");
      });
      expect(screen.getByLabelText("支付宝经营码内容")).toHaveValue(
        settings.collection!.code_payload,
      );
  ...
  it.each(["picker", "drop"])(
    "cancels %s decoding on section changes and ignores results after unmount",
    async (source) => {
      const pending = pendingDecode();
      vi.mocked(decodeQrImage).mockReturnValue(pending.promise);
      ...
      const signal = vi.mocked(decodeQrImage).mock.calls[0]![1];
      await act(async () => {
        await router.navigate("/settings/backup");
      });
      expect(signal.aborted).toBe(true);
      await act(async () => {
        pending.resolve(recognizedLink);
      ...
      expect(screen.queryByText("已识别")).not.toBeInTheDocument();
      await act(async () => {
        await router.navigate("/settings/collection");
      });
      expect(screen.getByLabelText("支付宝经营码内容")).toHaveValue(
        settings.collection!.code_payload,
      );
  ...
  it("protects a decoded draft when navigating away", async () => {
    ...
    const user = userEvent.setup();
    await user.upload(
      await screen.findByLabelText("选择经营码二维码图片"),
      imageFile(),
    );
    await screen.findByText("已识别");
  ...
  it("retains the decoded draft after a revision conflict", async () => {
    ...
    expect(screen.getByText("未保存")).toBeVisible();
    expect(await writes(fetchMock)[0]!.json()).toMatchObject({
      revision: 3,
      code_payload: recognizedLink,
    });
  });
  ...
  it("keeps the drag highlight across nested controls and clears it on leaving the target", async () => {
    ...
    const nestedControl = screen.getByRole("button", { name: "上传二维码" });
    const dataTransfer = { types: ["Files"], files: [], dropEffect: "none" };
    expect(screen.getByText("上传二维码")).toBeVisible();
    ...
    expect(zone).not.toHaveClass("bg-accent");
    expect(screen.getByText("上传二维码")).toBeVisible();
    expect(decodeQrImage).not.toHaveBeenCalled();
  });

  it.each(["text/plain", "text/uri-list"])(
    "leaves native %s dragging alone and never fetches dragged links",
    async (type) => {
      const { fetchMock } = mount();
      const field = await screen.findByLabelText("支付宝经营码内容");
      const dataTransfer = {
        types: [type],
        files: [],
        getData: () => recognizedLink,
      };
      const over = createEvent.dragOver(field, { dataTransfer });
      fireEvent(field, over);
      const drop = createEvent.drop(field, { dataTransfer });
      fireEvent(field, drop);
  ...
  it("rejects multiple dropped files without choosing one or replacing the draft", async () => {
    ...
    const field = await screen.findByLabelText("支付宝经营码内容");
    expect(
      dropFiles(uploadZone(), [imageFile(), imageFile()]).defaultPrevented,
    ).toBe(true);
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "一次只能识别一张二维码图片，请重新选择。",
    );
  ...
  it.each([
    ...
    "cancels an older decode after rejecting %i dropped files and ignores its late %s",
    async (count, outcome) => {
      const pending = pendingDecode();
      vi.mocked(decodeQrImage).mockReturnValue(pending.promise);
      const { fetchMock } = mount();
      ...
      expect(writes(fetchMock)).toHaveLength(0);
      fireEvent.change(screen.getByLabelText("收银台有效期（秒）"), {
        target: { value: "450" },
      });
      expect(screen.getByRole("button", { name: "保存" })).toBeEnabled();
    },
  ...
  it.each(["format", "size"])(
    "applies the existing image %s validation to dropped files",
    async (invalid) => {
      const actual =
        await vi.importActual<typeof import("../src/qr-image")>(
          "../src/qr-image",
        );
      ...
      const file =
        invalid === "format"
          ? new File(["pdf"], "code.pdf", { type: "application/pdf" })
  ...
  it("prevents accidental file navigation outside the target only while the field is mounted", async () => {
    ...
    fireEvent.dragEnter(zone, { dataTransfer });
    const outside = createEvent.dragOver(document.body, { dataTransfer });
    ...
    unmount();
    expect(dropFiles(document.body, [imageFile()]).defaultPrevented).toBe(
      false,
    );
  });
  ...
  it("respects the disabled fieldset while a configuration save is pending", async () => {
    ...
    expect(
      screen.getByRole("group", { name: "二维码图片上传" }),
    ...
    expect(field).toHaveValue(settings.collection!.code_payload);
    await act(async () => {
      finish(json({ data: { ...settings, revision: 4 } }));
    });
    expect(screen.getByLabelText("支付宝经营码内容")).toBeEnabled();
  });

  it.each(["{Enter}", " "])(
    "opens the picker with %s from the upload card without adding a duplicate tab stop",
    async (key) => {
      mount();
      const picker = await screen.findByLabelText("选择经营码二维码图片");
