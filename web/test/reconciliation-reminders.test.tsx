...
const cases = [
  {
    ...
    endpoint: "/api/admin/v1/reconciliation/exceptions",
    idKey: "exception_id",
    title: "收入尚未匹配订单",
  },
  {
    ...
    endpoint: "/api/admin/v1/ledger/conflicts",
    idKey: "conflict_id",
    title: "流水金额无法解析",
  },
] as const;
...
describe("reconciliation reminder navigation", () => {
  it.each(cases)(
    "refreshes the $tab list after ignoring and restoring its reminder",
    async (scenario) => {
      const user = userEvent.setup();
      let ignored = false;
      const requests: Request[] = [];
      const entry = {
        [scenario.idKey]: ledgerId,
      ...
      const reminder = {
        ...
        actionable_at: entry.created_at,
        ignored_at: null,
        ignored_by: null,
        ended: false,
      ...
      vi.stubGlobal(
        ...
        vi.fn(async (request: Request) => {
          ...
          if (
            request.method === "POST" &&
            url.pathname.endsWith("/ignore-all")
          ) {
            ignored = true;
            return json({
              data: { ...(await request.json()), ignored_count: 1 },
            });
          }
          if (
            request.method === "POST" &&
            url.pathname.endsWith("/actions/restore")
          ) {
            ignored = false;
            return json({
              data: {
          ...
          if (url.pathname === scenario.endpoint)
            return json({
              data:
                !ignored || url.searchParams.get("status") === "ALL"
                  ? [entry]
                  : [],
          ...
          if (url.pathname === "/api/admin/v1/work-items") {
            const selected =
              (url.searchParams.get("visibility") === "IGNORED") === ignored;
            return json({
              data: selected
                ? [
                    {
      ...
      render(
        <QueryClientProvider client={queryClient}>
          <MemoryRouter
            initialEntries={["/reconciliation?tab=" + scenario.tab]}
          >
            <nav>
              <Link to={"/reconciliation?tab=" + scenario.tab}>账本记录</Link>
              <Link to={"/work-items?type=" + scenario.type}>管理提醒</Link>
            </nav>
            <Routes>
      ...
      expect(
        await screen.findByRole("link", { name: scenario.title }),
      ).toBeVisible();
      ...
      expect(
        screen.queryByRole("link", { name: scenario.title }),
      ).not.toBeInTheDocument();
      if (scenario.tab === "conflicts") {
        expect(
          screen.getByRole("combobox", { name: "对账状态筛选" }),
        ).toHaveTextContent("未忽略");
        await user.click(screen.getByLabelText("对账状态筛选"));
        await user.click(
        ...
        expect(
          screen.getByRole("combobox", { name: "对账状态筛选" }),
        ).toHaveTextContent("全部记录");
        await user.click(screen.getByLabelText("对账状态筛选"));
        await user.click(await screen.findByRole("option", { name: "未忽略" }));
        await screen.findByText("暂无符合条件的记录");
      }
      const historyLink = screen.getByRole("link", { name: "查看已忽略" });
      expect(historyLink).toHaveAttribute(
        "href",
        "/work-items?type=" + scenario.type + "&visibility=IGNORED",
      );
      ...
      expect(
        requests.filter(
          (request) => new URL(request.url).pathname === scenario.endpoint,
        ).length,
