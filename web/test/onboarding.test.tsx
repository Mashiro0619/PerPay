...
const pathIs = (view: ReturnType<typeof mountOnboarding>, step: string) =>
  waitFor(() =>
...
describe("first collection onboarding", () => {
  ...
  it("keeps administrator initialization separate, then begins configuration after login", async () => {
    ...
    edit("设置管理员密码", "isolated-onboarding-password");
    edit("再次输入密码", "isolated-onboarding-password");
    ...
    edit("管理员密码", "isolated-onboarding-password");
    await user.click(screen.getByRole("button", { name: "登录" }));
    await pathIs(view, "application");
    expect(
      view.writes().map((request) => new URL(request.url).pathname),
    ).toEqual(["/api/admin/v1/setup", "/api/admin/v1/session/login"]);
  });
  ...
  it.each([0, 1, 2, 3, 4])(
    ...
    (stage) => {
      ...
      expect(resolveOnboardingStep(configuredThrough(stage), "unknown")).toBe(
        next,
      );
      expect(
        resolveOnboardingStep(configuredThrough(stage), "application"),
      ).toBe("application");
    },
  );
  ...
  it("keeps prerequisites in optional help without repeating long instructions", async () => {
    ...
    expect(
      screen.queryByText(/支付宝搜索“经营码”申请/),
  ...
  it("keeps the provider step focused on App ID and the Alipay public key", async () => {
    ...
    expect(
      screen.queryByText(/应用网关|生成密钥文件|保存只完成配置/),
    ).not.toBeInTheDocument();
    expect(screen.getAllByRole("heading", { name: "支付宝接入" })).toHaveLength(
      1,
    );
    expect(view.writes()).toHaveLength(0);
  });
  it.each(["/orders", "/settings/provider"])(
    "does not force explicit %s navigation through onboarding",
    async (path) => {
      const view = mountOnboarding({ path });
      await screen.findByRole("heading", {
        name: path === "/orders" ? "订单" : "实例设置",
        level: 1,
      });
  ...
  it("defers only this instance in the tab and clears the deferral on logout", async () => {
    ...
    expect(
      view.router.state.location.state?.deferOnboardingFor,
    ).toBeUndefined();
  ...
  it("preserves a conflicted draft and stays on the same step", async () => {
    ...
    await pathIs(view, "collection");
    expect(screen.getByLabelText("支付宝经营码内容")).toHaveValue(
      "https://qr.alipay.com/conflict-draft",
    );
  ...
  it("saves a manual collection payload with the current revision before continuing", async () => {
    ...
    expect(
      screen.queryByText("上传接入账户的经营码，核对识别结果后保存。"),
    ).not.toBeInTheDocument();
  ...
  it("preserves the other optional form while saving each with the latest revision", async () => {
    ...
    expect(await view.writes()[1]!.clone().json()).toEqual({
      revision: 4,
  ...
  it("retains a wizard draft during a temporary session query failure", async () => {
    ...
    const view = mountOnboarding({
      stage: 2,
      handle: (request) =>
        fail && new URL(request.url).pathname.endsWith("/session")
          ? apiError("internal_error", "temporary failure", 503)
          : undefined,
    });
    const field = await screen.findByLabelText("支付宝经营码内容");
    edit("支付宝经营码内容", "https://qr.alipay.com/retained");
    fail = true;
    await act(async () => {
  ...
  it("ignores a save callback after the user leaves the step", async () => {
    ...
    await screen.findByLabelText("支付宝经营码内容");
    edit("支付宝经营码内容", "https://qr.alipay.com/late");
    const user = userEvent.setup();
    ...
    await pathIs(view, "provider");
    await act(async () => {
      finish(json({ data: configuredThrough(3) }));
    });
    await pathIs(view, "provider");
  });
});
