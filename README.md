...
本项目使用 SQLite，不需要另设数据库密码。经营码、应用 ID、应用私钥、平台公钥、API 密钥和通知配置不要写进 Compose。填写完成后启动：
...
打开 `PERPAY_PUBLIC_URL`，首次访问会要求设置管理员密码；完成后使用该密码登录后台。在“设置”页面填写经营码、平台凭据、API 密钥和通知配置；管理员密码之后可在“安全”页面修改。
