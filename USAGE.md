...
| 设置 | 新配置默认值 | 使用时机 |
| --- | --- | --- |
| 常规采集间隔（`scan_interval_seconds`） | 60 秒 | 没有有效待支付订单，且收尾期已结束。 |
| 活跃采集间隔（`active_scan_interval_seconds`） | 8 秒 | 当前采集账户仍有未过期的 `OPEN / UNPAID` 订单，或有尚在收尾期的未支付订单。 |

两个间隔均为 5～3600 秒的整数，活跃间隔不能大于常规间隔；两者相同即保持固定频率。新配置的采集有效时限默认为 120 秒，须至少为常规间隔的两倍，安全延迟也不能超过该有效时限。保存后由运行时热更新，无须重启进程。
...
**升级不会自动改变已有采集频率：** 原间隔会同时成为常规和活跃间隔，例如原来的 10 秒会保留为 10／10 秒。希望启用 60／8 秒时，请主动调整后台设置。管理 API 更新支付宝配置时，若省略 `active_scan_interval_seconds`，两档均使用请求中的 `scan_interval_seconds`，以兼容旧客户端。
...
订单详情直接展示收款流水、付款确认方式、人工关联的操作人和理由，以及最新通知结果。精确匹配窗口在“匹配依据”中展开；记录旁的“更多 → 单独打开”仍可定位或分享具体记录。

- 收款、异常、通知默认给出可读摘要；原始字段、内部编号和指纹放在“技术详情”。金额推断不是支付平台按商户订单号确认，匹配候选也不等于订单当前已付款。
- 关联撤销只在实际撤销后展示“撤销时间”。人工关联可在当前页面执行；撤销关联、退款标记和通知重投递位于对应记录的“更多”中。已有订单及流水自动带入，打开后读取最新证据。高风险操作确认一次，仍保留必要理由。
- 已忽略和已处理异常归入可展开的“异常历史”。忽略只关闭提醒，不能据此认定原异常已解决。
- 通知的 HTTP 成功不等于业务已确认，需看 ACK 结果。只有最新且已结束的投递提供重投递入口；正在自动重试或已有后续投递的旧记录不提供此操作。
- 未配置通知时仅显示简短说明；未付款且无历史标记的订单不显示退款入口。退款标记只记录外部已完成退款的管理员声明。
...
- 切换到“已忽略”可查看原始详情，并逐条“恢复提醒”，恢复不再重复确认，不提供批量恢复。“账本与对账”的“查看已忽略”可进入同一列表；冲突的“全部记录”仍包含已忽略项。恢复后，未结束事项重新出现在未忽略列表。
...
先在 PerPay 之外自行完成退款，再到 **订单详情 → 概览右上角“更多” → 标记已退款**；误标时可以“撤销退款标记”。备注可选，最多 500 字，修改历史记录操作人、时间和备注。
...
点击后台侧栏的 **外观**，先选配色，再选 **浅色／深色／跟随系统**，立即生效，无需保存：

- 配色有经典蓝、青竹、暖砂、柔紫、石墨，每种都有浅色和深色。
- 点选时面板保持打开；点击外部、关闭按钮或按 Escape 收起。手机先打开导航菜单。
- 偏好仅保存在当前浏览器，会同步到同源的其他标签页。旧的浅色／深色偏好不变，默认经典蓝＋跟随系统。
- 登录和初始化页面使用同一外观，不影响公开收银台、表单草稿或任何账务数据。

### 设置与安全

常用配置默认展示；采集间隔、超时和重试参数放在“高级设置”，校验失败会自动展开并定位。普通设置未修改时不能重复保存，配置向导仍可保存默认值并继续。离开未保存表单时会确认是否放弃。

“密钥与安全”按用途列出密钥。点击 **查看** 即读取，60 秒后、关闭弹窗或切换标签页会清除；不会随页面进入或切换主题读取明文。API 密钥轮换位于该行“更多”中。轮换后旧密钥立即失效，需同步业务网站；结果不确定时不要重复轮换，先刷新配置并查看当前密钥。

修改密码需输入两次，以“修改并重新登录”提交；注销全部会话位于“登录会话”的“更多”中。
