...
订单详情直接展示收款流水、金额与时间窗口依据、人工关联的操作人和理由，以及最新业务通知结果。无需沿着内部编号逐页跳转；“打开独立详情”仍可定位或分享具体记录。

- 收款、异常、通知默认给出可读摘要；原始字段、内部编号和指纹放在“技术详情”。金额推断不是支付平台按商户订单号确认，匹配候选也不等于订单当前已付款。
- 关联撤销只在实际撤销后展示“撤销时间”。适用的人工关联、撤销、退款标记和通知重投递可在当前页面执行；已有订单及流水自动带入，仍须核对证据和确认危险操作。
