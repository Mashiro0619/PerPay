...
const config = { url: origin, secret: secret.toString('base64url'), port: 6196, database: ':memory:', notifyUrl: null, webhookSecret: secret };
...
describe('standalone PerPay caller demo', () => {
  ...
  it('requires both callback settings and never accepts a local HTTP notification URL', () => {
    const env = { PERPAY_URL: origin, PERPAY_API_SECRET: secret.toString('base64url') };
    assert.equal(readConfig(env).notifyUrl, null);
    assert.throws(() => readConfig({ ...env, DEMO_NOTIFY_URL: 'https://shop.example/webhooks/perpay' }));
    assert.throws(() => readConfig({ ...env, DEMO_NOTIFY_URL: 'http://127.0.0.1/webhooks/perpay', PERPAY_WEBHOOK_SECRET: secret.toString('base64url') }));
  });
