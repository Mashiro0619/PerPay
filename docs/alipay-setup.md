...
回到 PerPay，填入同一应用的 **App ID** 和 **支付宝公钥**，点击 **“保存并继续”**。默认使用生产环境，高级设置可保持不变。新部署的数据库初始值与首次配置表单保持一致：常规采集间隔 60 秒、活跃采集间隔 8 秒、采集有效时限 120 秒，均可调整；升级后的已有配置保留原值。详见[账本采集节奏](../USAGE.md#账本采集节奏)。
