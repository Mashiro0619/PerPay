...
- 目标44项通过；完整npm run check后端762/前端693通过，3项平台跳过。隔离实例21项浏览器场景通过、0页面错误：1440/390/320×明暗×三图形、常驻滚动条配置、125%/150%布局缩放及长数值、加载态；实测数值client/scroll尺寸并人工看图。截图和日志在本任务admin-ui-revisions目录part-1-*。其余三段的最终验收记录见下文。
...
### 修订3：侧栏轻提示与共享运行状态
- 沿用[官方SidebarMenuBadge](https://ui.shadcn.com/docs/components/base/sidebar)及Alert/Item组合。首页移除非阻断“可以收款，有运行告警”横幅，侧栏运行状态显示轻量“待处理/需关注”；正常无标记。暂停新收款、离线/未知状态仍显式提示。
- 一个受鉴权边界保护的状态提供者供侧栏、概览和运行状态页复用，页面可见时30秒轮询、隐藏时停、切回/路由返回重新读取；失效结果不继续冒充正常。只改前端展示，未修改服务端status或收款准入。
- 冲突/异常/死信积压继续显示真实计数和入口，但不单独把健康采集、自动确认染成运行故障。真正工作器错误、追赶/停止、备份恢复风险和未知degraded原因仍提示。全量回归发现旧会话夹具返回不完整200响应会牵连表单，补充响应完整性检查与回归，降为未知而非崩溃或重置草稿。
- 69项既有目标回归、新增9项状态场景及会话生命周期回归通过；完整npm run check后端767/前端711通过，3项平台跳过。60项隔离浏览器场景通过、0错误：1440/390/320×明暗×正常/积压/运行告警/阻断/读取失败×概览/运行页；逐页验证仅一次初始状态读取及侧栏可达性，人工看图复核。日志part-3-targeted.log、part-3-state-tests-final.log、part-3-check-final.log和part-3-browser.json。

### 修订4：独立差异列与手机逐字段对照
- 使用[官方Table](https://ui.shadcn.com/docs/components/base/table)与[Item / ItemHeader](https://ui.shadcn.com/docs/components/base/item)组合，未改动共享UI原语。桌面为字段／传入记录／已有流水／差异四列；无已有流水时为字段／传入记录／校验结果三列，仅说明一次“无可对照流水”。对照区域上限1024px，字段128px、结果80px，两侧证据均分余下宽度。
- 以对照容器640px而非整页宽度切换布局。手机每个字段先显示名称与独立结果，再依次显示传入值、已有值；分组具名，隐藏布局不进入可访问树。不同项用图标与“不同”，异常原值用“异常”，其余弱化为“—”，不在字段名下堆叠徽标。
- 保留1.001、原始交易时间及毫秒精度、长编号与多行备注；只改变显示，不改规范化规则或任何财务判定。采集摘要直显，原始报文仍按需查看，处理理由及原操作恢复保护不变。
- 人工复核发现早期手机卡片虽然具备flex-col，却与Item默认flex-wrap、ItemHeader的basis-full冲突，标题占满了整列，把证据挤到右边。移除这个不必要的方向覆盖，沿用原语的整行标题与换行内容；补充组件布局契约及具名分组断言。浏览器进一步断言标题在上、证据占满下一行、双方上下排列与全部字段无溢出，并注入旧样式做负向对照，确认旧错误确实会被验收拒绝。
- 91项目标回归通过；完整npm run check后端767通过／3项平台跳过，前端53文件715项通过，类型、API生成一致性和构建均通过。未升应用版本，未增加数据库迁移。
- 本段最终31项浏览器验收、0页面错误：1920/1440/390/320×明暗×有无已有流水16场景，1440/390/320×明暗×有无已有流水的长证据12场景，639/640容器边界及1项旧布局负向对照。检查列宽、逐字段几何、可访问顺序、异常原值、多行与精度、技术详情及处理确认；逐一复核代表性桌面和手机截图。长证据组合仅覆盖隔离实例的GET响应，不伪造实际支付事实。

### 四段修订最终验收与预览
- 前三段独立提交为cfbbed7、6594953、4116868；第四段在本次验收后独立提交。四段全部完成，不推送、不升版；应用仍为0.2.2，数据库兼容范围仍为24—26，公开收银台不改。
- 在同一最终构建上重跑四段浏览器验收：21＋32＋60＋31＝144项，0页面错误。隔离HTTP服务启动时会缓存前端资源，因此重建后显式重启，并比对服务返回与磁盘index.html的脚本文件名，避免把旧页面结果算作最终验收。以final-browser-runs.json及build-fingerprint.json为最终依据，早期运行不作为本轮完成依据。
- 最终日志、脚本、可访问树和截图目录：`C:/Users/Mashiro/.codex/visualizations/2026/09/23/01a0ceb9-b470-7e73-b292-8cfbb2b7ccff/admin-ui-revisions-final`。目标回归为part-4-targeted-final.log，完整检查为part-4-check.log，冲突几何记录为part-4-browser.json；其余三段也已在此目录保留最新结果。
- 业务写入验收均在隔离合成库中执行。原预览库先做只读完整性检查及SQLite在线备份，再使用原数据库、原账号和配置恢复6233后端及6191前端；没有初始化新管理员、造订单或修改财务数据。恢复前后除进程租约和启动元数据时间外，逐表内容摘要一致，启动元数据实际值不变。预览未启动真实支付宝采集或通知传输，既有图表示例历史仍仅存在于外部预览响应中。
