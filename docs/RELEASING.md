...
- 正式容器只发布稳定语义版本 `X.Y.Z`，不发布 `latest`。
...
- 正式部署以 OCI digest 为字节身份。版本标签用于发现版本，Release 附件中的 `tag@sha256:...` 用于固定实际运行字节。
...
1. 同步更新以下版本位置：
   ...
   - `src/version.ts`
   - `Dockerfile`
   - `docker-compose.yml` 中 `app`、`backup` 与 `maintenance` 的三处镜像标签
...
7. 从根 Compose 生成 Release 附件。附件中 `app`、`backup` 与 `maintenance` 使用完全相同的 `X.Y.Z@sha256:...` 引用，并通过 `docker compose config --quiet` 以及显式启用 `maintenance` profile 的镜像引用检查；默认配置仍只包含两个长期服务。
8. 在 GitHub 托管的 Linux runner 上使用该 digest 固定的 Compose，实际验证应用健康、付款未就绪状态、非 root 与只读容器约束、SQLite 备份、备份列表、数据库恢复、容器重建和两个命名卷的持久化。
9. 最后创建 GitHub Release，附加 `docker-compose.yml`，在说明中记录镜像 digest、支持平台、数据库 schema 范围和 Compose SHA-256，并且不把该 Release 自动标记为 Latest。
...
GHCR 没有被本流程依赖的原子 create-if-absent/CAS 标签写入。在“确认 `X.Y.Z` 不存在”与“创建 `X.Y.Z`”之间，外部 package writer 理论上仍可抢占或移动该标签。workflow 的并发组只能阻止同一 Git ref 的本工作流互相覆盖，不能约束工作流之外的写入者。
...
2. 确认三处 `image:` 完全相同，且同时包含版本标签和顶层 digest。
3. 填写配置，运行：
...
OCI digest 一经公布即为该版本的字节权威。需要修复时发布新的补丁版本，不得重打同版本镜像，也不得删除仍受支持版本的 Compose、版本说明或恢复资料。
